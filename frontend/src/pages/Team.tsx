...
const Team: React.FC = () => {
  ...
  return (
    <div className="mx-auto w-full flex flex-col gap-8 min-h-full animate-in fade-in duration-700 pb-10">
      ...
      <div className="card-standard p-0 border border-border overflow-hidden bg-card shadow-sm rounded-2xl">
              ...
              <tbody className="divide-y divide-zinc-100">
                {members.map((member, idx) => (
                  <tr key={member.id} className="hover:bg-muted transition-colors group">
                    <td className="px-6 py-4 text-sm font-bold text-zinc-500 text-center">
                    ...
                    <td className="px-6 py-4">
                      <span className={`text-[10px] uppercase font-bold tracking-wider px-2.5 py-1 rounded-md ${member.role === 'Client Admin' ? 'bg-sky-100 text-sky-700' : 'bg-secondary text-zinc-500'
                        }`}>
                        {member.role || 'Agent'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      {member.role !== 'Client Admin' && (
                        <button
                          ...
                          className="p-2 text-zinc-500 hover:bg-red-600 hover:text-white rounded-lg transition-all opacity-0 group-hover:opacity-100 focus:opacity-100 inline-flex"
                          title="Remove member"
                        >
                          <Trash2 size={18} />
                        </button>
