...
import { Mail, Lock, LogIn, AlertCircle, Loader2, Eye, EyeOff, MessageCircle, MoreHorizontal, Info, CheckCircle2, DollarSign, Database, Calendar } from 'lucide-react';
...
const LaptopMockup = () => {
  ...
  // Steps:
  // 0: Empty
  // 1: Customer msg appears
  // 2: Flazly typing...
  // 3: Flazly msg appears
  // 4: Notif 1 (Lead Captured)
  // 5: Notif 2 (Contact Saved)
  // 6: Notif 3 (CRM Updated)
  // 7: Notif 4 (Meeting Booked)
  // 8: Notif 5 (Revenue)
  // 9-11: Hold and glow
    ...
    <div className="relative w-full max-w-[800px] px-8 md:px-16 flex flex-col items-center justify-center transform scale-95 xl:scale-100 transition-transform duration-500">
      ...
      <div className="relative z-20 w-full aspect-[16/10] bg-[#0c0c0c] rounded-t-[16px] md:rounded-t-[24px] border-[6px] md:border-[12px] border-[#18181b] border-b-[2px] md:border-b-[4px] shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex-1 bg-black flex relative">
          ...
          {/* Sidebar (Optional - to make it look like desktop IG) */}
        ...
        <div className="absolute top-16 left-64 right-0 bottom-0 pointer-events-none p-6 flex flex-col items-end justify-end pb-20 gap-3 z-40 overflow-hidden">
           <AnimatePresence>
      ...
      <div className="w-full h-full flex flex-col md:flex-row relative">
        ...
        <div className="w-full md:w-[45%] lg:w-[40%] p-8 md:p-12 flex flex-col items-center justify-center relative bg-[#080808] z-20 border-r border-white/5 shadow-2xl">
          <div className="w-full max-w-[340px] mx-auto flex flex-col items-center">
            ...
            <h1 className="text-2xl font-semibold text-primary-foreground mb-2 text-center tracking-tight">Sign in to Flazly</h1>
            <p className="text-sm font-medium text-muted-foreground mb-8 text-center">Your best sales rep never sleeps.</p>
            ...
            {!isPending2Fa ? (
                ...
                <form onSubmit={handleLogin} className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between ml-1">
                      <label className="text-xs font-semibold text-zinc-300 block" htmlFor="password">Password</label>
                      <a href="#" className="text-xs font-medium text-muted-foreground hover:text-primary-foreground transition-colors">Forgot password?</a>
                    </div>
                    <div className="relative group">
              ...
              <form onSubmit={handleVerify2Fa} className="space-y-4 w-full">
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-muted-foreground block text-center" htmlFor="totpCode">Two-Factor Authentication Code</label>
                  <div className="relative group">
                    <Lock size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground z-10" />
                    <input
                      type="text"
                      id="totpCode"
                      className="w-full pl-11 pr-4 py-3 bg-primary/80 border border-primary rounded-2xl text-primary-foreground text-sm focus:border-white/20 outline-none transition-all font-mono tracking-widest text-center"
                      placeholder="000000"
                      value={totpCode}
                ...
                <button type="submit" className="w-full bg-white hover:bg-zinc-200 text-black rounded-full py-3 font-bold text-sm transition-all flex items-center justify-center gap-2" disabled={isLoading}>
                  {isLoading ? <Loader2 size={16} className="animate-spin" /> : 'Verify Code'}
                </button>

                <button type="button" onClick={() => { setIsPending2Fa(false); setMessage(null); }} className="w-full bg-transparent hover:bg-primary text-muted-foreground rounded-full py-3 font-medium text-sm transition-all border border-primary">
                  Cancel
                </button>
              </form>
            )}
        ...
        <div className="hidden md:flex w-full md:w-[55%] lg:w-[60%] flex-col items-center justify-center relative overflow-hidden bg-[#0a0a0c]">
          ...
          <div className="z-10 w-full flex flex-col items-center justify-center mb-8">
            <LaptopMockup />
          </div>
        </div>
      </div>
