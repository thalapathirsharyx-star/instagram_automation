    ...
    <div className="flex-1 space-y-4 p-4 pt-6 md:p-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 border-b border-border pb-6">
        <div>
          ...
          <p className="text-zinc-500 text-sm">Manage and track your AI-discovered Instagram pipeline.</p>
        </div>
      ...
      <div className="border bg-card text-card-foreground shadow-sm rounded-xl overflow-hidden flex flex-col relative">
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap">
            <thead>
              <tr className="bg-muted/50 border-b border-border">
                <th className="h-10 px-4 text-left align-middle font-medium text-zinc-500">Customer Profile</th>
                <th className="h-10 px-4 text-left align-middle font-medium text-zinc-500">Pipeline Status</th>
                <th className="h-10 px-4 text-left align-middle font-medium text-zinc-500">AI Score</th>
                <th className="h-10 px-4 text-left align-middle font-medium text-zinc-500">Detected Intent</th>
                <th className="h-10 px-4 text-left align-middle font-medium text-zinc-500 text-right">Actions</th>
              </tr>
            </thead>
                ...
                <tr>
                  <td colSpan={5} className="py-24 text-center">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-zinc-500 text-sm">Connect your Instagram account or adjust search filters.</p>
                    </div>
                  </td>
                </tr>
                ...
                <tr key={lead.id} className="hover:bg-muted transition-colors duration-200 group cursor-pointer" onClick={() => setSelectedLead(lead)}>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3 w-32">
                      ...
                      <span className="text-xs font-bold text-zinc-500">{Number(lead.lead_score || 0).toFixed(0)}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
        ...
        {!isLoading && filteredLeads.length > 0 && (
          <div className="px-6 py-4 border-t border-border bg-muted/50 flex justify-between items-center text-xs font-medium text-zinc-500">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-primary/40 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setSelectedLead(null)}>
          <div className="w-full max-w-2xl bg-card border border-border rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-6 py-5 border-b border-border flex justify-between items-center bg-muted/50">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-brand flex items-center justify-center text-foreground font-bold text-xl border border-primary/20 shadow-sm shrink-0">
            ...
            <div className="p-6 md:p-8 space-y-8 overflow-y-auto max-h-[70vh]">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-5 rounded-2xl bg-muted rounded-lg border flex flex-col justify-center">
                  ...
                  <p className="text-3xl font-extrabold text-foreground tracking-tight">{Number(selectedLead.lead_score || 0).toFixed(0)}<span className="text-sm text-zinc-500 font-medium ml-1">/ 10</span></p>
                </div>
              ...
              <div className="space-y-3">
                ...
                <div className="p-5 rounded-xl bg-card border border-border text-zinc-500 leading-relaxed font-medium text-sm shadow-sm">
              ...
              {selectedLead.tags && selectedLead.tags.length > 0 && (
                <div className="space-y-3">
                  <h3 className="text-xs font-bold text-foreground uppercase tracking-wider flex items-center gap-1.5"><Tag size={14} className="text-zinc-500"/> CRM Tags</h3>
                  <div className="flex flex-wrap gap-2">
                    {selectedLead.tags.map((tag, idx) => (
                      <span key={idx} className="px-2.5 py-1 bg-secondary text-zinc-500 text-[11px] font-bold uppercase tracking-wider rounded border border-border">
                        {tag}
                      </span>
                    ))}
                  </div>
