              ...
              <div key={i} style={{ fontSize: '0.85rem', padding: '12px', borderBottom: '1px solid var(--glass-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  ...
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-dim)', marginTop: '2px' }}>
                    <span className={`px-1.5 py-0.5 rounded text-[8px] uppercase font-bold mr-2 ${client.plan !== 'Free' ? 'bg-brand/10 text-brand border border-brand/20' : 'bg-primary/90 text-zinc-500'}`}>
                      {client.plan || 'Free'}
                    </span>
                    {client.email}
                  </div>
