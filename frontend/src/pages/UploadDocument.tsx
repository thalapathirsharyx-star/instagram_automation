    ...
    <div className="flex flex-col gap-8 min-h-full animate-in fade-in duration-700 pb-10">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-zinc-500 font-medium">Upload a PDF or TXT file to extract knowledge facts.</p>
        </div>
      </div>
      ...
      <div className="card-standard p-10 border-brand/20 bg-primary animate-in slide-in-from-bottom-4 duration-500 shadow-xl max-w-3xl">
        <div 
          ...
          className={`border-2 border-dashed border-zinc-200 rounded-3xl p-16 text-center ${!isSaving ? 'cursor-pointer hover:border-brand/50 hover:bg-brand/5' : ''} transition-all duration-300 group`}
        >
          <input 
            type="file" 
            ref={fileInputRef} 
          ...
          {isSaving ? (
            ...
            <>
              <div className="w-20 h-20 bg-brand/10 rounded-3xl flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform shadow-[0_0_20px_rgba(168,85,247,0.2)]">
                <UploadCloud size={40} className="text-brand" />
              </div>
              <h4 className="text-xl font-bold text-zinc-100 mb-2">Drop your file here</h4>
              <p className="text-zinc-500 font-medium max-w-sm mx-auto">Supports PDF and TXT. AI will process the text into searchable knowledge.</p>
            </>
          )}
        </div>
      </div>
